...
- **概览**：Steam 路径、版本、Lua 数量、DLL 状态、设置摘要、项目信息。
- **Lua**：导入 Lua、打开 Lua 目录、启用/禁用 Lua、编辑 AppId、Token、Manifest、Depot Key 等。
- **DLL**：查看 DLL 资源哈希、目标哈希和 SHA-256 匹配状态。
- **设置**：编辑日志等级、清单来源、HTTP timeout、额外 Lua 路径、签名镜像。
- **日志**：读取 `<Steam>\opensteamtool\*.log`。
- **关于**：项目版本、作者和仓库地址。
...
3. 确认 `src-tauri/resources/dlls` 或打包资源中存在：
   - `OpenSteamTool.dll`
   - `dwmapi.dll`
   - `xinput1_4.dll`
4. 点击 **初始化**，应用会安装 DLL 并写入默认 `opensteamtool.toml`。
5. 在 Lua 页面新增或导入游戏配置。
6. 重启 Steam，使 DLL 和 TOML 配置生效。
